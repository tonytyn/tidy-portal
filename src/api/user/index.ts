...
import type { SearchUserParam, UserListResult, CreateUserParam, UpdateUserParam } from './models'
...
export const searchUserListApi = (searchUserParam: SearchUserParam) => {
  ...
  return defRequest.get<ResopnseResult<PageResult<UserListResult>>>(
    '/user?username=' + searchUserParam.username + '&pageNum=' + searchUserParam.pageNum + '&pageSize=' + searchUserParam.pageSize
  )
}
...
export const deleteUserApi = (id: number) => {
  return defRequest.delete<ResopnseResult<null>>('/user/delete/' + id)
}
